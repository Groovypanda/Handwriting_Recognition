...
def plot_images(images, cls_true, cls_pred=None):
    ...
    for i, ax in enumerate(axes.flat):
        ...
        if cls_pred is None:
            xlabel = "True: {0}".format(utils.cls2str(cls_true[i]))
        else:
            xlabel = "True: {0}, Pred: {1}".format(utils.cls2str(cls_true[i]), utils.cls2str(cls_pred[i]))
...
def plot_weights(weights):
    ...
    for i, ax in enumerate(axes.flat):
        if i < 62:
            ...
            ax.set_xlabel("Heatmap {}".format(utils.cls2str(i)))
