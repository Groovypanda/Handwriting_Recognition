...
import character_extraction as chrextr
import definitions
...
def open_images(start=0, amount=1000):
    file_entries = [x.split(' ') for x in
                    open(definitions.WORDSET_INFO_PATH, 'r').read().splitlines()[
...
def find_start(file_name):
    file_entries = [x.split(' ') for x in
                    open(definitions.WORDSET_INFO_PATH, 'r').read().splitlines()[START_LINE:]]
