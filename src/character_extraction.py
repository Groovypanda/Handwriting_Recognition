...
def extract_characters(word_image, sessionargs=None, postprocess=True):
    """
    ...
    chosen_split_layout = extract_character_separations(word_image, sessionargs=sessionargs, postprocess=postprocess)
...
def find_splits_img(image):
    ...
    new_ranges = [[(split, nr) for split_range in combined_range for (split, nr) in split_range] for combined_range in
                  combined_ranges]
