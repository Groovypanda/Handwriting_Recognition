...
\begin{algorithmic}
    ...
    \IF{ 
    \\ 
    \hspace{8px} $xGap < text$.averageWordWith 
    \\
    \hspace{8px} $* $ \hspace{2px} $(1 / (averageCharacterCount * 2))$ 
    \\
    {\bfseries and} 
    \\ 
    \hspace{8px} $yOverlap > min(contour1.height, contour2.height)$ 
    \\
    \hspace{8px} $* $ \hspace{2px} $  requiredYaxisOverlap $ 
    }
    \vspace{4px}
...
\begin{algorithmic}
   ...
   \REPEAT
        ...
        \REPEAT
            ...
            \ENDIF
        \ENDFOR
        \UNTIL{NotChangedLastIteration $line$}
...
More advanced text segmentation approaches can be utilized to extract the text segments from documents containing images and other unwanted elements. \cite{textsegm}
...
More postprocessing can be used through rule based techniques and a more robust neural network to correct the found segmentation points. \cite{segmtechniques}
