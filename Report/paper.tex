...
\begin{algorithmic}
    ...
    \IF{ 
    $xGap < avgWidth * (1 / 8 * 2)$ \\  {\bfseries and} \\ $yOverlap > min(contour1.height, contour2.height) / 2 $ 
    }
...
After oversegmenting the word, we find the actual splitpoints by removing unnecessary splitpoints as explained in \ref{sec:postprocesscharseg}
...
The binarization method is identical to \ref{sec:binartext} but included to allow segmentation of words which are not part of a text. Binarization is essential as it is necessary for skeletonization.
...
We interpret these columns as whitespace in between characters or as a line connecting two characters to each other.
...
An example of applying these correction techniques to an image is shown in \ref{fig:char_segm}.
...
Finally we aggregate groups of consequent potential segmentation points, separated by columns containing less pixels than a threshold value 3.
...
A comparison of different text segmentation techniques is done by \cite{textsegm}.

The character segmentation process can be improved upon by increasing the accuracy of the used neural network, and by adding more techniques to remove incorrect segmentations. Several such rule-based techniques are explored in \cite{segmtechniques}.
