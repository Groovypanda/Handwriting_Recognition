...
To be able to recognise the characters in a given text, we first need to extract the words from the text before we can start evaluating the characters. Word extraction from text images has many possible approaches, but in this paper we will focus solely on the word contouring approach, pointed out by following paper \cite{WordSegm} as being a good word extraction approach.

This heuristic approach will extract the contours of the words from a given text image, and through postprocessing we can rearrange these extracted words into lines.
The proposed approach focusses solely text images, and does not take into account images and other non-textual areas found in the text.
Dependent on the situation, additional techniques for page segmentation can be utilized. % , such as the Run-Length Smearing Algorithm, can be utilized.
